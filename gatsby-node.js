...
function getAssetsForChunks(chunks) {
  const files = _.flatten(chunks.map(chunk => readStats().assetsByChunkName[chunk]));
...
exports.onPostBuild = (args, {
  ...
  const rootDir = `public`; // Get exact asset filenames for app and offline app shell chunks

  const files = getAssetsForChunks([`app`, `webpack-runtime`, `component---node-modules-gatsby-plugin-offline-app-shell-js`]);
  ...
  return workboxBuild.generateSW({
    ...
    if (debug !== undefined) {
      const swText = fs.readFileSync(swDest, `utf8`).replace(/(workbox\.setConfig\({modulePathPrefix: "[^"]+")}\);/, `$1, debug: ${JSON.stringify(debug)}});`);
      fs.writeFileSync(swDest, swText);
    }

    const swAppend = fs.readFileSync(`${__dirname}/sw-append.js`, `utf8`).replace(/%pathPrefix%/g, pathPrefix).replace(/%appFile%/g, appFile);
