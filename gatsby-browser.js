exports.registerServiceWorker = () => true; // only cache relevant resources for this page
...
exports.onServiceWorkerActive = ({
  ...
  const headerResources = [].slice.call(nodes) // don't include preconnect/prefetch/prerender resources
  .filter(node => node.tagName !== `LINK` || whiteListLinkRels.test(node.getAttribute(`rel`))).map(node => node.src || node.href || node.getAttribute(`data-href`)); // Loop over prefetched pages and add their resources to an array,
