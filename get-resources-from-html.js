...
module.exports = (htmlPath, pathPrefix) => {
  ...
  const $ = cheerio.load(html); // holds any paths for scripts and links
  ...
  `).each((_, elem) => {
    const $elem = $(elem);
    const url = $elem.attr(`src`) || $elem.attr(`href`) || $elem.attr(`data-href`); // Don't cache XML files, or external resources (beginning with // or http)

    const blackListRegex = /(\.xml$|^\/\/|^http)/; // check resource URLs from header tags start with the correct prefix
